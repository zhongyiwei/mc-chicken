<div id="xxx" style="display:none">
    <ul class="container">


                <li class="menu">

            <ul>
                <li class="button"><a href="#" class="green">Coat of Arms <span></span></a></li>

                <li class="dropdown">
                    <ul>
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio_coa_motto/#MS" >Why & How?</a></li>
                        <!--<li><a href="<?php echo base_url();?>index.php/home/portfolio_motto/" >My Motto</a></li>-->
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio/#COA" >My Coat of Arms</a></li>

                    </ul>
                </li>

            </ul>

        </li>



        <li class="menu">

            <ul>
                <li class="button"><a href="#" class="green">My Records <span></span></a></li>

                <li class="dropdown">
                    <ul>
                        <!--<li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#MOTTO">My Motto</a></li>-->
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#MS">My Mission</a></li>
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#VALUE">My Values</a></li>
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#ACHIEVEMENT">My Achievement</a></li>
                        <li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#RESILIENCE">My Resilience Scales</a></li>
                        <!--<li><a href="<?php echo base_url();?>index.php/home/portfolio_mission/#MS">My Progress Status</a></li>-->
                        <!--<li><a href="<?php //echo base_url();?>index.php/home/reminder_setting/">Your Reminder Setting</a></li>-->

                    </ul>
                </li>

            </ul>

        </li>


    </ul></div>
<!-- end #sidebar -->
<div style="clear: both;">&nbsp;</div>
</div>
<!-- end #page -->
<div id="home">
    <a href="<?php echo base_url();?>index.php/home/index/"><img src="<?php echo base_url();?>CSS/images/background/home_button.png" alt="Home"/></a>
</div>
<div id="footer" align="center">
    <div id="image_footer"><a href="#"><img src="<?php echo base_url();?>CSS/images/background/what_is_now.png" alt="" /></a></div>
    <div id="image_footer"><a href="<?php echo base_url();?>index.php/home/portfolio_coa_motto/#MS"><img src="<?php echo base_url();?>CSS/images/background/do_u_know.png" alt="" /></a></div>